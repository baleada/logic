...
export function createToCommonAncestors<
  Id extends string,
  Metadata
> (graph: Graph<Id, Metadata>): GraphNodeTupleGeneratorFn<Id, GraphCommonAncestor<Id>> {
  const toNodeSteps = createToNodeSteps(graph)
...
export function createPredicateAncestor<
  Id extends string,
  Metadata
> (graph: Graph<Id, Metadata>): GraphNodeTupleFn<Id, boolean> {
  const toNodeSteps = createToNodeSteps(graph)
...
> (
  ...
  return function* (directedAcyclic) {    
    const { nodes } = directedAcyclic,
          ...
          roots = pipe(
            createToRoots<Id, Metadata>({ kind }),
            toArray(),
          )(directedAcyclic),
...
export function createToRoots<
  Id extends string,
  Metadata
> (options: { kind?: 'directed acyclic' | 'arborescence' } = {}): GraphGeneratorFn<Id, Metadata, GraphNode<Id>> {
  return function* (directedAcyclic) {
    const { nodes } = directedAcyclic
...
> (directedAcyclic: Graph<Id, Metadata>): GraphStateFn<Id, Metadata, GraphNode<Id>[]> {
  const toOutdegree = createToOutdegree(directedAcyclic),
        toOutgoing = createToOutgoing(directedAcyclic),
