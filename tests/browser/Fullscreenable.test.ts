...
suite('stores the getElement', async ({ puppeteer: { page } }) => {
  const value = await page.evaluate(() => {
          const instance = new (window as unknown as WithLogic).Logic.Fullscreenable(() => document.body)
...
suite('assignment sets the getElement', async ({ puppeteer: { page } }) => {
  const value = await page.evaluate(() => {
          const instance = new (window as unknown as WithLogic).Logic.Fullscreenable(() => document.querySelector('span') as HTMLElement)
...
suite(`status is 'errored' after unsuccessful fullscreen(...)`, async ({ puppeteer: { page } }) => {
  const value = await page.evaluate(async () => {
          const instance = new (window as unknown as WithLogic).Logic.Fullscreenable(() => ({}))
          await instance.fullscreen()
          return instance.status
        }),
        expected = 'errored'
